...
@patch("backend.src.services.ingestion._fetch_captions_via_timedtext")
...
@patch("backend.src.services.ingestion._fetch_captions_via_timedtext", return_value=None)
@patch("backend.src.services.ingestion._fetch_captions_via_ytdlp", return_value=None)
def test_hybrid_falls_back_to_metadata(mock_ytdlp, mock_timedtext):
    ...
    with patch.object(HybridIngestionService, "enrich", wraps=service.enrich):
        with patch("backend.src.services.ingestion.VideoIndexerService") as vi_cls:
